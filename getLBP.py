...
class LBP:
    ...
    def describe(self,image):
        image_array=np.array(Image.open(image).resize((128,128)).convert('L'))
        return image_array
...
def getLBP(img):
    ...
    image_array = cv2.cvtColor(reImg, cv2.COLOR_BGR2GRAY)
    # image_array = np.array(Image.open(image).resize((128,128)).convert('L'))
    
    return lbp.lbp_basic(image_array)

if __name__ == '__main__':
    image = r"F:\SC\test3.jpg";
    lbp=LBP()
